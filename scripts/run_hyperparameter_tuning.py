...
from src.config import SUMMARIES_DIR, RESULTS_MODEL_DIR, MODEL_FEATURES, DATASET_ALL, MIN_TRAIN_RACES, CV_N_SPLITS, CV_N_JOBS, BEST_PARAMS_FILE, RANDOM_STATE
...
from src.modeling.analysis_plots import plot_model_comparison


def make_objective(model_name, X, y):
    tscv = TimeSeriesSplit(n_splits=CV_N_SPLITS)

    # define objective function for Optuna with conditional hyperparameters
    def objective(trial):
...
if __name__ == "__main__":
    ...
    log(summary_lines, "Hyperparameter Tuning\n")

    log(summary_lines, f"Trials: {args.trials}")

    for model_name in MODELS_TO_TUNE:
        log(summary_lines, f"\n{'-'*45}")
        log(summary_lines, f"Tuning: {model_name}")
        ...
        log(summary_lines, f"Tuning time: {tune_time:.1f} s")
        ...
        log(summary_lines, f"\nWalk-forward validation (best params)")
        if model_name == "RandomForest":
            model = Pipeline([
                ('imputer', SimpleImputer(strategy='median')),
                ('model', RandomForestRegressor(**best_params, n_jobs=-1, random_state=RANDOM_STATE)),
            ])
        elif model_name == "XGBoost":
            model = XGBRegressor(**best_params, n_jobs=-1, random_state=RANDOM_STATE, verbosity=0)
        elif model_name == "LightGBM":
            model = LGBMRegressor(**best_params, n_jobs=-1, random_state=RANDOM_STATE, verbose=-1)
        elif model_name == "CatBoost":
        ...
        log(summary_lines, f"Time: {wf_time:.1f} s")
