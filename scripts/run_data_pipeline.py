...
from src.config import CACHE_DIR, DATA_PROCESSED_DIR, SUMMARIES_DIR, LOGS_DIR, ERRORS_LOG, TRACK_PARAMETERS_FILE, RESULTS_CLUSTERING_VERIFICATION_DIR
...
from src.data_preparation.physics import build_static_features
from src.data_preparation.pirelli import build_pirelli_features
...
if __name__ == "__main__":
            ...
            try:
                ...
                for attempt in range(3):
                    try:
                        ...
                        if attempt == 2:
                            raise
                        wait = 5 * (3 ** attempt) # retry backoff
                        logging.warning(f"{YEAR} {location}: session.load() attempt {attempt+1} failed: {e}, retry again in {wait}s")
                        time.sleep(wait)
                ...
                df_track = build_static_features(session, df_weather, df_pirelli)
                df_track = build_pirelli_features(df_track)
    ...
    if full_dataset:
        ...
        if 'Style_Cluster_ID' in df_all.columns:
            ...
            log(summary_lines, "Clustering columns missing")
