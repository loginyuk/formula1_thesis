...
from src.telemetry.curvature import compute_curvature

logger = logging.getLogger('clustering')

def add_curvature_to_telemetry(tel):
    """
    Calculate curvature for each telemetry point and add it as a new column
    """
    ...
    tel['Curvature'] = compute_curvature(x, y)
...
def extract_corner_features(corner_tel):
    ...
    speed_variability = np.std(speed) / entry_speed
    ...
    for drv in laps['Driver'].unique():
        ...
        try:
            ...
            if 'Distance' not in full_tel.columns:
                full_tel = full_tel.add_distance()
        ...
        for _, lap in drv_laps.iterrows():
            try:
                ...
                for corner_id, (start_m, end_m) in corner_zones.items():
                    corner_tel = lap_tel[(lap_tel['Distance'] >= start_m) & (lap_tel['Distance'] <= end_m)].copy()
                    metrics = extract_corner_features(corner_tel)
                    if metrics is None:
                        continue
                    
                    # save metrics with identifiers
                    metrics['Driver'] = drv
