...
def compute_metrics(actual, predicted):
    """
    Get common regression metrics between actual and predicted values
    """
...
def shift_telemetry_features(df, features_to_shift=None):
    """
    Shifts all telemetry-derived features by 1 lap
    """
    ...
    print(f"Rows remaining after shifting: {len(df)}")
    return df


def run_season_walk_forward(df, features, model, summary_lines, target='LapTime_Sec', min_train_races=5, print_progress=True):
    """
    Runs walk-forward validation across the season, 
    training on all previous races and testing on the next one
    """
    ...
    for i in range(min_train_races, len(races)):
        ...
        predictions_log.append(test_data[['Year', 'RoundNumber', 'Location', 'Location_Encoded', 
    ...
    if print_progress:
        ...
        log(summary_lines, f"Walk-forward validation time: {end_time - start_time:.1f} seconds")
